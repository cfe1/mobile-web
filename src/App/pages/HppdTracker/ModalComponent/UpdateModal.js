...
const UpdateModal = ({
  ...
  const [pageSize, setPageSize] = useState(10); // const [jobTitleSelectedValues, setjobTitleSelectedValues] = useState([]);
  const [JobTitleArrayMapped, setJobTitleArrayMapped] = useState([]);

  useEffect(() => {
    setJobTitleArrayMapped(
      jobTitleSelectedValues.map((item) => item.job_title_id)
    );
  }, []);
  ...
  const onCloseEvent = () => {
    setHppdDataModal([]);
    setjobTitleSelectedValues([]); // setJobTitleArray([]);
    setisOpenNew(false);
  };
