...
import calenderIcon from "App/assets/icons/calendar-2.svg";
...
const DateRangePicker = ({
  ...
  const [isPickerSettingStartDate, setIsPickerSettingStartDate] =
    useState(true);
  ...
  const handleKeyDown = (e) => {
    ...
    const underscoreIndexPosition = value.indexOf("_");

    if (
      Number(e.keyCode) === 8 &&
      underscoreIndexPosition > -1 &&
      selectionStart === 10
    ) {
  ...
  const getDateRangetxt = () => {
    return (
      <div onClick={openPicker} className={classes.dateTxt}>
        {`${moment(initialDateRange?.startDate).format(
          "MMM DD, YYYY"
        )} - ${moment(initialDateRange?.endDate).format("MMM DD, YYYY")}`}

        <img src={calenderIcon} className={classes.calenderIcon} />
      </div>
    );
  };
  const openPicker = () => {
                ...
                <Grid item xs>
                  {
                    <TextField
                      ...
                      placeholder={dateStringFormatter.toLowerCase()}
                      fullWidth
                    />
                  }
                </Grid>
